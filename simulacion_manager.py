...
def generar_simulacion():
    ...
    # ── 400 FACTURAS CON ÉXITO: 20 PROVEEDORES × 20 FACTURAS ─────────────────
    print("  [GENERANDO] Creando 400 facturas procesadas (20 proveedores x 20 facturas cada uno)...")
    contador = 0
    for s_idx, supplier in enumerate(PROVEEDORES_VALIDOS, 1):
        # Cada proveedor tiene un rango de importes propio (coherente al filtrar)
        base_amount = random.uniform(35, 850)
        for n in range(1, 21):
            contador += 1
            # Reparto estable de meses: 20 facturas repartidas entre los 12 meses
            month_str = TODOS_MESES[(n - 1) % len(TODOS_MESES)]
            year_full = f"20{month_str.split('-')[1]}"
            amount = round(base_amount * random.uniform(0.65, 1.55), 2)
            ...
            entry_id = f"sim_p{s_idx:02d}_{n:02d}"
            simulated_history.append({
                ...
                "supplier": supplier,
                "date_str": month_str,
                "drive_file_id": f"drive_sim_file_{contador:04d}",
                "drive_folder_id": f"drive_folder_prov_{contador:04d}",
    ...
    print("  • Procesadas con éxito: 400 (20 proveedores x 20 facturas cada uno)")
